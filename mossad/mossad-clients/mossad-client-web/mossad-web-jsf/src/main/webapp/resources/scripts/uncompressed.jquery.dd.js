...
;(function ($) {
  ...
  var dd = function (element, options) {
    ...
    ddList = ($(sElement).prop("size") > 1 || $(sElement).prop("multiple")
        == true) ? true : false;
    if (ddList) {
    ...
    var createA = function (currentOptOption, current, currentopt, tp) {
      ...
      var aidoptfix = (tp == "opt") ? getPostID("postOPTAID") : getPostID(
          "postAID");
      var aid = (tp == "opt") ? aidoptfix + "_" + (current) + "_" + (currentopt)
          : aidoptfix + "_" + (current);
      var arrow = "";
      var clsName = "";
      if (options.useSprite != false) {
        ...
        arrow = (arrow.length == 0) ? "" : '<img src="' + arrow
            + '" align="absmiddle" /> ';
      }
      ;
      var sText = $(currentOptOption).text();
      var sValue = $(currentOptOption).val();
      var sEnabledClass = ($(currentOptOption).prop("disabled") == true)
          ? "disabled" : "enabled";
      a_array[aid] = {
        html: arrow + sText,
        value: sValue,
      ...
      if (matchIndex(currentOptOption.index) == true) {
        aTag += '<a href="javascript:void(0);" class="' + styles.selected + ' '
            + sEnabledClass + clsName + '"';
      } else {
      ...
      ;
      aTag += ' id="' + aid + '">';
      aTag += arrow + '<span class="' + styles.ddTitleText + '">' + sText
          + '</span></a>';
      return aTag;
    };
    ...
    var createChildDiv = function () {
      ...
      if (!ddList) {
        ...
        sDiv += (sStyle != "")
            ? ' style="border-top:1px solid #c3c3c3;display:block;position:relative;'
            + sStyle + '"' : '';
      }
      ;
      sDiv += '>';
    ...
    var createTitleDiv = function () {
      ...
      ;
      arrow = (arrow.length == 0 || arrow == undefined || options.showIcon
          == false || options.useSprite != false) ? "" : '<img src="' + arrow
          + '" align="absmiddle" /> ';
      var sDiv = '<div id="' + titleid + '" class="' + styles.ddTitle + '"';
      sDiv += '>';
      sDiv += '<span id="' + arrowid + '" class="' + styles.arrow
          + '"></span><span class="' + styles.ddTitleText + '" id="'
          + titletextid + '">' + arrow + '<span class="' + styles.ddTitleText
          + '">' + sText + '</span></span></div>';
      return sDiv;
    };
    var applyEventsOnA = function () {
      ...
      $("#" + childid + " a.enabled").bind("click", function (event) {
        ...
        if (!ddList) {
          ...
          var sText = (options.showIcon == false) ? $(this).text() : $(
              this).html();
          setTitleText(sText);
          //$this.data("dd").close();
          $this.close();
    ...
    var createDropDown = function () {
      ...
      if (allOptions.length > options.visibleRows) {
        var margin = parseInt(
            $("#" + childid + " a:first").css("padding-bottom")) + parseInt(
            $("#" + childid + " a:first").css("padding-top"));
    ...
    var manageSelection = function (obj) {
      ...
      if (selectedA != undefined) {
        var oldIndex = (actionSettings.oldIndex == undefined
            || actionSettings.oldIndex == null) ? a_array[selectedA].index
      ...
      if (ddList) {
        ...
        if ($("#" + elementid).prop("multiple") == true) {
          if (keyCode == 17) {
            ...
            $("#" + childid + " a." + styles.selected).removeClass(
                styles.selected);
            $(obj).addClass(styles.selected);
            //shift
            var currentSelected = $(obj).prop("id");
            var currentIndex = a_array[currentSelected].index;
            for (var i = Math.min(oldIndex, currentIndex);
                i <= Math.max(oldIndex, currentIndex); i++) {
    ...
    var setOutOfVision = function () {
      var sId = getPostID("postElementHolder");
      $("#" + elementid).after("<div class='" + styles.ddOutOfVision
          + "' style='height:0px;overflow:hidden;position:absolute;' id='" + sId
          + "'></div>");
    ...
    var navigateA = function (w) {
      ...
      var currentP = $("#" + childid + " a:visible").index(
          $("#" + childid + " a.selected:visible"));
      var nextA;
      switch (where) {
        case "next":
      ...
      if (where == "next") {
        if (parseInt(
            ($("#" + selectedA).position().top + $("#" + selectedA).height()))
            >= parseInt($("#" + childid).height())) {
          $("#" + childid).scrollTop(
              ($("#" + childid).scrollTop()) + $("#" + selectedA).height() + $(
              "#" + selectedA).height());
        }
        ;
      } else {
        if (parseInt(
            ...
            <= 0) {
          $("#" + childid).scrollTop(
              ($("#" + childid).scrollTop() - $("#" + childid).height()) - $(
              "#" + selectedA).height());
        }
        ;
      }
    ...
    var setTitleImageSprite = function (i) {
      if (options.useSprite != false) {
        var titletextid = getPostID("postTitleTextID");
        var index = (typeof (i) == "undefined") ? getElement(
        ...
        if (sClassName.length > 0) {
          ...
          if (backgroundImg != undefined) {
            $("#" + titletextid).find("." + styles.ddTitleText).attr('style',
                "background:" + backgroundImg);
          }
          ;
          if (backgroundPosition != undefined) {
            $("#" + titletextid).find("." + styles.ddTitleText).css(
    ...
    var checkMethodAndApply = function () {
      ...
      if (has_handler('change') == true) {
        ...
        var currentSelectedValue = a_array[$(
            "#" + childid + " a.selected").prop("id")].text;
    ...
    var setOriginalProperties = function () {
      ...
      for (var i in getElement(elementid)) {
        if (typeof (getElement(elementid)[i]) != 'function' && getElement(
            elementid)[i] !== undefined && getElement(elementid)[i] !== null) {
    ...
    /************* public methods *********************/
    this.open = function () {
      if (($this.get("disabled", true) == true) || ($this.get("options",
          true).length == 0)) {
        return;
      }
      var childid = getPostID("postChildID");
      ...
      if ($("#" + childid).css("display") == "none") {
        oldSelectedValue = a_array[$("#" + childid + " a.selected").prop(
            "id")].text;
        //keyboard action
        var sText = "";
        ...
        $(document).bind("keydown", function (event) {
          ...
          if (keyCode == 8) {
            ...
            event.stopPropagation();
            //remove char
            sText = (sText.length == 0) ? "" : sText.substr(0,
                sText.length - 1);
          }
          ;
          switch (keyCode) {
    ...
    this.debug = function (is) {
      if (typeof (is) == "undefined" || is == true) {
        ...
        $("." + styles.ddOutOfVision).attr("style",
    ...
    this.set = function (prop, val, isLocal) {
      //alert("- set " + prop + " : "+val);
      if (prop == undefined || val
          == undefined) {
        throw {message: "set to what?"};
      }
      ...
      if (isLocal != true) {
        switch (prop) {
          ...
          case "multiple":
            getElement(elementid)[prop] = val;
            ddList = ($(sElement).prop("size") > 0 || $(sElement).prop(
    ...
    this.add = function (opt, index) {
      ...
      var sValue = (objOpt.value == undefined || objOpt.value == null) ? sText
          : objOpt.value;
      var img = (objOpt["title"] == undefined || objOpt["title"] == null) ? ''
          : objOpt["title"];
      var i = (index == undefined || index == null) ? getElement(
          elementid).options.length : index;
      getElement(elementid).options[i] = new Option(sText, sValue);
      if (img != '') {
        getElement(elementid).options[i]["title"] = img;
      }
      //check if exist
    ...
    //return form element
    this.form = function () {
      return (getElement(elementid).form == undefined) ? null : getElement(
          elementid).form;
    };
    this.item = function () {
