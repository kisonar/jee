  ...
  $.fn.slides = function (option) {
    ...
    return this.each(function () {
      ...
      var elem = $(this),
          ...
          effect = option.effect.indexOf(',') < 0 ? option.effect
              : option.effect.replace(' ', '').split(',')[0],
          paginationEffect = option.effect.indexOf(',') < 0 ? effect
              : option.effect.replace(' ', '').split(',')[1],
          next = 0, prev = 0, number = 0, current = 0, loaded, active, clicked,
          position, direction, imageParent, pauseTimeout, playInterval;

      // is there only one slide?
      if (total < 2) {
        // Fade in .slides_container
        $('.' + option.container, $(this)).fadeIn(option.fadeSpeed,
      ...
      function animate(direction, effect, clicked) {
        if (!active && loaded) {
          ...
          if (option.pagination) {
            ...
            $('.' + option.paginationClass + ' li:eq(' + next + ')',
      ...
      $('.' + option.paginationClass + ' li:eq(' + start + ')', elem).addClass(
          option.currentClass);
